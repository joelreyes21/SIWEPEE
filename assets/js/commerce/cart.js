...
<<<<<<< HEAD
  function sync(){
    /* Identidad de la tienda robusta: id numérico si lo tenemos, si no el slug.
       Buscamos los items por id O por slug (deEmpresa acepta ambos), así el
       carrito no "sale vacío" si por algún motivo DB.empresa_id llega en 0. */
    ...
    items=siwepeCart.deEmpresa(clave).map(x=>{
      ...
      if(!p||p.estado!=='activo'||Number(p.stock)<=0)return null;
      // Nunca sobreescribimos un empresa_id válido con 0.
      return{...x,empresa_id:eid||Number(x.empresa_id)||0,empresa_slug:slug||x.empresa_slug||'',empresa_nombre:DB.config.nombre,nombre:p.nombre,precio:Number(p.precio_venta)||0,imagen:p.imagen||x.imagen||'',stock:Number(p.stock)||0,cantidad:Math.min(Math.max(1,Number(x.cantidad)||1),Number(p.stock)||1)};
    }).filter(Boolean);
    // Solo reescribimos el storage si el catálogo cargó (hay productos): así un
    // fallo transitorio de carga no borra un carrito válido.
  ...
  function render(){const list=$('#cart-list'),summary=$('#cart-summary'),count=items.reduce((n,x)=>n+x.cantidad,0),total=items.reduce((n,x)=>n+x.precio*x.cantidad,0);if(!items.length){list.innerHTML=`<div class="cart-empty"><div><svg><use href="#c-bag"/></svg><h2>Tu carrito está vacío</h2><p>Explorá el catálogo y agrega los productos que te gusten.</p><a class="commerce-primary" href="${url('tienda.html')}#catalogo">Ver catálogo</a></div></div>`;summary.hidden=true;return}summary.hidden=false;list.innerHTML=items.map(x=>`<article class="cart-item" data-id="${x.producto_id}">${x.imagen?`<img class="cart-image" src="${esc(x.imagen)}" alt="">`:`<span class="cart-image-placeholder">${esc(x.nombre[0])}</span>`}<div class="cart-copy"><h2>${esc(x.nombre)}</h2><p>${money(x.precio)} c/u · ${x.stock} disponibles</p><button data-remove><svg><use href="#c-trash"/></svg>Eliminar</button></div><div class="qty-control"><button data-delta="-1" aria-label="Restar">−</button><span>${x.cantidad}</span><button data-delta="1" aria-label="Sumar">+</button></div><strong class="cart-price">${money(x.precio*x.cantidad)}</strong></article>`).join('');$('#summary-count').textContent=`${count} artículo${count!==1?'s':''}`;$('#summary-subtotal').textContent=$('#summary-total').textContent=money(total)}
  function change(id,delta){const x=items.find(y=>y.producto_id===id);if(!x)return;const next=x.cantidad+delta;if(next>x.stock){toast('Llegaste al máximo disponible');return}siwepeCart.update(x.empresa_id,id,next);sync();render()}
  async function init(){if(!bsEmpresa()){location.href='index.html';return}await bootstrapDB({asBuyer:true});applyBrand();sync();render();$('#cart-list').addEventListener('click',e=>{const card=e.target.closest('[data-id]');if(!card)return;const id=Number(card.dataset.id);if(e.target.closest('[data-remove]'))change(id,-999);const d=e.target.closest('[data-delta]');if(d)change(id,Number(d.dataset.delta))});$('#go-checkout').onclick=()=>{if(!items.length)return;if(!bsToken()||!['cliente','admin'].includes(bsRole())){try{sessionStorage.setItem('siwepe_after_login',url('checkout.html'))}catch(e){}location.href=`${url('tienda.html')}&login=1`;return}location.href=url('checkout.html')}}
=======
  function sync(){items=siwepeCart.deEmpresa(DB.empresa_id).map(x=>{const p=(DB.productos||[]).find(y=>Number(y.id)===Number(x.producto_id));if(!p||p.estado!=='activo')return null;const vs=(p.variantes||[]).filter(v=>v.activo!==false),v=vs.find(v=>String(v.id)===String(x.variante_id||''));if(vs.length&&!v)return null;const stock=Number(v?v.stock:p.stock)||0;if(stock<=0)return null;return{...x,empresa_id:Number(DB.empresa_id),empresa_slug:String(ref()),empresa_nombre:DB.config.nombre,nombre:p.nombre,variante_id:v?String(v.id):'',variante_nombre:v?Object.values(v.atributos||{}).filter(Boolean).join(' · '):'',precio:Number(v?v.precioVenta:p.precio_venta)||0,imagen:p.imagen||x.imagen||'',stock,cantidad:Math.min(Math.max(1,Number(x.cantidad)||1),stock)}}).filter(Boolean);const others=siwepeCart.get().filter(x=>Number(x.empresa_id)!==Number(DB.empresa_id));siwepeCart.set([...others,...items])}
