(() => {
  ...
  function loadItems(){items=siwepeCart.deEmpresa(DB.empresa_id).map(x=>{const p=(DB.productos||[]).find(y=>Number(y.id)===Number(x.producto_id));if(!p||p.estado!=='activo')return null;const vs=(p.variantes||[]).filter(v=>v.activo!==false),v=vs.find(v=>String(v.id)===String(x.variante_id||''));if(vs.length&&!v)return null;const stock=Number(v?v.stock:p.stock)||0;if(stock<=0)return null;return{...x,nombre:p.nombre,variante_id:v?String(v.id):'',variante_nombre:v?Object.values(v.atributos||{}).filter(Boolean).join(' · '):'',precio:Number(v?v.precioVenta:p.precio_venta)||0,imagen:p.imagen||x.imagen||'',stock,cantidad:Math.min(Math.max(1,Number(x.cantidad)||1),stock)}}).filter(Boolean);const others=siwepeCart.get().filter(x=>Number(x.empresa_id)!==Number(DB.empresa_id));siwepeCart.set([...others,...items])}
  function renderSummary(){const count=items.reduce((n,x)=>n+x.cantidad,0),total=items.reduce((n,x)=>n+x.precio*x.cantidad,0);$('#checkout-count').textContent=`${count} artículo${count!==1?'s':''}`;$('#checkout-subtotal').textContent=$('#checkout-total').textContent=money(total);$('#checkout-items').innerHTML=items.map(x=>`<div class="checkout-item">${x.imagen?`<img src="${esc(x.imagen)}" alt="">`:`<span>${esc(x.nombre[0])}</span>`}<div><h3>${esc(x.nombre)}</h3>${x.variante_nombre?`<small>${esc(x.variante_nombre)}</small>`:''}<p>${x.cantidad} × ${money(x.precio)}</p></div><strong>${money(x.precio*x.cantidad)}</strong></div>`).join('')}
  ...
  async function submit(ev){
    ...
    if(metodo==='tarjeta'){
      toast('Esta tienda todavía no tiene una pasarela de tarjeta conectada.',true); return;
    }
    busy=true;
    ...
    try{
      ...
      const data=await apiPostAuth('/api/pedidos/checkout',{
        items:items.map(x=>({empresa_id:x.empresa_id,producto_id:x.producto_id,varianteId:x.variante_id||'',cantidad:x.cantidad})),
        nota:$('#order-note').value.trim(),metodoPago:metodo,
        comprobante:metodo==='transferencia'?proof:'',entrega
      });
