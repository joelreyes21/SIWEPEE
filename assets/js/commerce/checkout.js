(() => {
  ...
  function loadItems(){
    ...
    items=siwepeCart.deEmpresa(clave).map(x=>{const p=(DB.productos||[]).find(y=>Number(y.id)===Number(x.producto_id));if(!p||p.estado!=='activo')return null;const vs=(p.variantes||[]).filter(v=>v.activo!==false),v=vs.find(v=>String(v.id)===String(x.variante_id||''));if(vs.length&&!v)return null;const stock=Number(v?v.stock:p.stock)||0;if(stock<=0)return null;return{...x,empresa_id:eid||Number(x.empresa_id)||0,empresa_slug:slug||x.empresa_slug||'',nombre:p.nombre,variante_id:v?String(v.id):'',variante_nombre:v?Object.values(v.atributos||{}).filter(Boolean).join(' · '):'',precio:Number(v?v.precioVenta:p.precio_venta)||0,imagen:p.imagen||x.imagen||'',stock,cantidad:Math.min(Math.max(1,Number(x.cantidad)||1),stock)}}).filter(Boolean);
